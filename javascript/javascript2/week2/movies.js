...
//#1 Shortest Movie Titles:

const shortMovieTitle = movies.filter((movies) => movies.title.length < 5);
console.log(shortMovieTitle);

//#2 longest Movie Title:

const longMovieTitle = movies.filter((movies) => movies.title.length > 60);
...
const tagArray = movies.map((movies) => {
  if (movies.rating >= 7) {
    return "good";
  }
...
//#5 Using chaining,to filter & map movies rating:
...
const moviesKeywords = movies.filter((movies) => {
  const specialKeywords = movies.title.toLowerCase().split(" ");
  if (
    specialKeywords.includes("alien") ||
    specialKeywords.includes("benjamin") ||
    specialKeywords.includes("surfer")
  ) {
    return true;
  } else {
    return false;
...
console.log(moviesKeywords.length);


//#7 Word Duplicate in titles:

const wordSplit = movies.map(movie=> movie.title.toLowerCase().trim().split(/[^a-z0-9]+/gi))
// console.log(wordSplit);
function duplicateWords() {
  const wordSplitMap = wordSplit.map(movie => movie
    .filter((item, index) => movie
      .indexOf(item) !== index))
  return wordSplitMap
}
//console.log(duplicateWords());
const filteredDuplicateWords = duplicateWords().filter(word => word.length > 0)
console.log(filteredDuplicateWords);